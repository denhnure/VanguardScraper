...
class VanguardPuppeteerScraper {
    constructor() {
        ...
        // Email configuration using Gmail App Password (simple method)
        ...
        this.recipientEmail = process.env.RECIPIENT_EMAIL; // Who receives the email
    }
    ...
    async sendEmail(data) {
        try {
            if (!this.recipientEmail || !process.env.EMAIL_USER || !process.env.EMAIL_PASSWORD) {
            ...
            // Email content
            const emailSubject = `Vanguard Fund Update - ${data.nav_price_gbp || 'N/A'} (${data.change || 'N/A'})`;
            const htmlContent = `
<h2>Vanguard LifeStrategy 100 Equity Fund - Latest Data</h2>

<table style="border-collapse: collapse; width: 100%; font-family: Arial, sans-serif;">
    <tr style="background-color: #f2f2f2;">
        <td style="border: 1px solid #ddd; padding: 12px; font-weight: bold;">NAV Price (GBP)</td>
        <td style="border: 1px solid #ddd; padding: 12px;">£${data.nav_price_gbp || 'Not found'}</td>
    </tr>
    <tr>
        <td style="border: 1px solid #ddd; padding: 12px; font-weight: bold;">Change</td>
        <td style="border: 1px solid #ddd; padding: 12px; color: ${data.change && data.change.includes('+') ? 'green' : 'red'};">
            ${data.change || 'Not found'}
        </td>
    </tr>
    <tr style="background-color: #f2f2f2;">
        <td style="border: 1px solid #ddd; padding: 12px; font-weight: bold;">Last Updated</td>
        <td style="border: 1px solid #ddd; padding: 12px;">${new Date().toLocaleString()}</td>
    </tr>
</table>
...
This email was automatically generated by the Vanguard Fund Scraper using Gmail SMTP.
</p>
            `;
            ...
            const mailOptions = {
                ...
                attachments: [
                    {
                        filename: 'vanguard_fund_data.json',
            ...
            // Don't throw error - email failure shouldn't stop the scraper
        }
    }
}
